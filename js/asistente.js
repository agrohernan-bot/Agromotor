...
async function iaEnviar() {
  ...
  try {
    ...
    // Llamar al proxy seguro (nunca expone la API key)
    ...
    if (data.ia_remaining !== undefined) iaActualizarContador(data.ia_remaining, data.ia_total);
...
function iaActualizarContador(remaining, total) {
  var el = $('ia-counter');
  if (!el) return;
  if (remaining === -1) { el.style.display = 'none'; return; }
  el.style.display = 'block';
  var pct = total > 0 ? remaining / total : 0;
  var color = pct > 0.4 ? 'rgba(74,140,92,.6)' : pct > 0.15 ? '#C8A255' : '#C0392B';
  el.style.color = color;
  el.textContent = remaining > 0
    ? remaining + ' de ' + total + ' consultas disponibles este mes'
    : 'Límite mensual alcanzado · Se reinicia el próximo mes';
}
